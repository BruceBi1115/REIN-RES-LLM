...
init_common_defaults() {
  ...
  NEWS_DROPOUT="${NEWS_DROPOUT:-0.10}"
  ...
  DELTA_SIGN_EXTERNAL_PATIENCE="${DELTA_SIGN_EXTERNAL_PATIENCE:-6}"
  ...
  DELTA_SIGN_EXTERNAL_NEWS_DROPOUT="${DELTA_SIGN_EXTERNAL_NEWS_DROPOUT:-1}"
  ...
  UTILITY_RERANK_ENABLE="${UTILITY_RERANK_ENABLE:-1}"
  ...
  DELTA_OTHER_LR_SCALE="${DELTA_OTHER_LR_SCALE:-2.0}"
  DELTA_ROUTE_BALANCE_LAMBDA="${DELTA_ROUTE_BALANCE_LAMBDA:-0.02}"
  DELTA_ROUTE_ABSTAIN_LAMBDA="${DELTA_ROUTE_ABSTAIN_LAMBDA:-0.05}"
  DELTA_ROUTE_ABSTAIN_TARGET="${DELTA_ROUTE_ABSTAIN_TARGET:-0.35}"
  DELTA_ROUTE_CONF_FLOOR="${DELTA_ROUTE_CONF_FLOOR:-0.25}"
...
configure_dataset_defaults() {
  case "$DATASET_KEY" in
    nas14)
      ...
      DELTA_TEMPORAL_TEXT_FREEZE_ENCODER=1
    ...
    nsw_load)
      ...
      DEFAULT_NEWS_PATH="dataset/news_2024_2025.json"
      ALLOW_AUTO_DISCOVER_CACHE=1
      ...
      DELTA_SIGN_EXTERNAL_HIDDEN="${DELTA_SIGN_EXTERNAL_HIDDEN:-192}"
      DELTA_STRUCTURED_FEATURE_DIM="${DELTA_STRUCTURED_FEATURE_DIM:-12}"
      DELTA_SIGN_EPS="${DELTA_SIGN_EPS:-0.05}"
      DELTA_HEAD_LR_SCALE="${DELTA_HEAD_LR_SCALE:-1.5}"
    ...
    nsw_price)
      ...
      DEFAULT_NEWS_PATH="dataset/news_2024_2025_elecprice.json"
      ALLOW_AUTO_DISCOVER_CACHE=1
      ...
      DELTA_SIGN_EXTERNAL_HIDDEN="${DELTA_SIGN_EXTERNAL_HIDDEN:-192}"
      DELTA_STRUCTURED_FEATURE_DIM="${DELTA_STRUCTURED_FEATURE_DIM:-12}"
      DELTA_SIGN_EPS="${DELTA_SIGN_EPS:-0.08}"
      DELTA_HEAD_LR_SCALE="${DELTA_HEAD_LR_SCALE:-1.5}"

      case "$PROFILE_KEY" in
        default|relative_text)
          PROFILE_KEY="relative_text"
          DELTA_TEMPORAL_TEXT_ENABLE="${DELTA_TEMPORAL_TEXT_ENABLE:-1}"
          DELTA_TEMPORAL_TEXT_DIM="${DELTA_TEMPORAL_TEXT_DIM:-32}"
          DELTA_TEMPORAL_TEXT_MAX_LEN="${DELTA_TEMPORAL_TEXT_MAX_LEN:-196}"
        ...
        additive)
          ...
          DELTA_TEMPORAL_TEXT_PER_STEP_TOPK="${DELTA_TEMPORAL_TEXT_PER_STEP_TOPK:-3}"
          DELTA_RESIDUAL_MODE="${DELTA_RESIDUAL_MODE:-additive}"
          TASK_NAME_BASE="${TASK_NAME_BASE:-[2024-nswelecPRICE-tinynews_2]}"
          ;;
        *)
          echo "[ERROR] Unsupported profile=$PROFILE for dataset=$DATASET_KEY" >&2
          exit 1
          ;;
      esac
      ;;
...
apply_loop_overrides() {
  if [[ -n "${LOOKBACK_WINDOWS_OVERRIDE:-}" ]]; then
    read -r -a LOOKBACK_WINDOWS <<< "$LOOKBACK_WINDOWS_OVERRIDE"
  fi
  if [[ -n "${HORIZONS_OVERRIDE:-}" ]]; then
    read -r -a HORIZONS <<< "$HORIZONS_OVERRIDE"
  fi
  if [[ -n "${LRS_OVERRIDE:-}" ]]; then
    read -r -a LRS <<< "$LRS_OVERRIDE"
  fi
  if [[ -n "${BASE_BACKBONES_OVERRIDE:-}" ]]; then
    read -r -a BASE_BACKBONES <<< "$BASE_BACKBONES_OVERRIDE"
  fi
}
...
build_common_args() {
  ...
  COMMON_ARGS+=(
    ...
    --delta_route_balance_lambda "$DELTA_ROUTE_BALANCE_LAMBDA"
    --delta_route_abstain_lambda "$DELTA_ROUTE_ABSTAIN_LAMBDA"
    --delta_route_abstain_target "$DELTA_ROUTE_ABSTAIN_TARGET"
    --delta_route_conf_floor "$DELTA_ROUTE_CONF_FLOOR"
