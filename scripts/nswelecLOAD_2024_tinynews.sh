...
# Stable DELTA default for LOAD: keep one main residual branch and avoid
# extra fusion/gating paths until the residual setup is verified.
FINAL_GATE_ENABLE="${FINAL_GATE_ENABLE:-1}"   # 1 => use horizon-aware final fusion gate
DISABLE_ALL_GATES="${DISABLE_ALL_GATES:-0}"   # 1 => bypass internal delta gate + final gate + text gate
DELTA_GATE_INIT_BIAS="${DELTA_GATE_INIT_BIAS:--2.0}"  # negative => internal delta gate starts conservative
...
DELTA_SIGN_EPS="${DELTA_SIGN_EPS:-0.02}"
...
DELTA_RELATIVE_DENOM_FLOOR="${DELTA_RELATIVE_DENOM_FLOOR:-1.0}"
